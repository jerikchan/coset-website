...
export default class Test extends Component {
  ...
  constructor(props) {
    ...
    this.handleCarouselBodyMouseOver =
      this.handleCarouselBodyMouseOver.bind(this)
    this.handleCarouselBodyMouseOut = this.handleCarouselBodyMouseOut.bind(this)
    this.handleCarouselFooterMouseOver =
      this.handleCarouselFooterMouseOver.bind(this)
  ...
  handlerCarousel(type) {
    ...
    if (
      this.state.currentCarousel % (this.props.children.length + 1) !==
        this.props.children.length &&
      this.state.currentCarousel >= 0
    ) {
      ...
      this.setState((pre) => {
        ...
        return {
          ...
          currentCarousel:
            pre.currentCarousel % (this.props.children.length + 1),
        }
      })
    }
  }
  ...
  getIndicatorsActive(index) {
    ...
    if (
      ...
      this.state.currentCarousel === index + this.props.children.length ||
      (this.state.currentCarousel < 0 &&
          ...
          {this.props.children.map((item, index) => {
            ...
            return (
              <li
                ...
                className={`indicators-item ${active}`}
                key={index}
              ></li>
            )
          })}
      ...
      >
        <div
          ...
          style={{
            ...
            width: `${(this.props.children.length + 2) * 100}%`,
            transform: `translateX(${
              (-100 / (this.props.children.length + 2)) *
              (this.state.currentCarousel + 1)
            }%)`,
          }}
        >
          <div
        ...
        <div className="btn-container">
          <div className="btn-direction pre" onClick={this.handlerPre}>
            {"<"}
          </div>
          <div className="btn-direction next" onClick={this.handlerNext}>
            {">"}
          </div>
        </div>
      </div>
