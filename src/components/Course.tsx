  ...
  return(
    <div className="bg-web-paper text-gray-800 font-inter min-h-screen">
      ...
      <div className="lg:hidden sticky top-0 z-20 bg-white/90 backdrop-blur-sm border-b border-gray-200 py-3 px-4">
        ...
        >
          <span>{courseData.episodes[parseInt(activeSection.split('-')[1]) - 1] || '课程目录'}</span>
        ...
        >
          ...
          <div className="p-4 max-h-[calc(100dvh-4rem)] overflow-y-auto">
            <ul className="space-y-2 mb-8">
              {courseData.episodes.map((episode, idx) => (
                <li 
                  key={`episode-${idx + 1}`}
                  style={{
                    opacity: 0,
                ...
                >
                  <a 
                    href={`#episode-${idx + 1}`}
                    onClick={(e) => {
                      setActiveSection(`episode-${idx + 1}`);
                      closeMobileMenu();
                    }}
                    className={`block py-3 px-4 rounded-lg transition-all duration-200 ${
                      activeSection === `episode-${idx + 1}`
                ...
                <ul className="space-y-2">
                  {courseData.sponsors.map((sponsor, idx) => (
                    <li 
                      ...
                      style={{
                        opacity: 0,
                        animation: mobileMenuOpen ? `slideDown 0.4s ease-out ${(courseData.episodes.length + idx) * 50}ms forwards` : 'none'
                      }}
                    >
                      <a 
        ...
        <aside className="hidden lg:block w-80 shrink-0 pr-6 sticky top-20 self-start max-h-[calc(100vh-6rem)] overflow-y-auto custom-scrollbar">
          <div className="rounded-lg shadow-sm border border-gray-200 p-6 backdrop-blur-sm">
            ...
            <ul className="space-y-1 mb-8">
              {courseData.episodes.map((episode, idx) => (
                <li key={`episode-${idx + 1}`}>
                  <a 
                    href={`#episode-${idx + 1}`}
                    onClick={() => setActiveSection(`episode-${idx + 1}`)}
                ...
                <ul className="space-y-2">
                  {courseData.sponsors.map((sponsor, idx) => (
                    <li key={`sponsor-nav-${idx}`}>
                      ...
                      >
                        <span className="flex items-center">
                          {/* <span className="w-4 h-4 flex-shrink-0 rounded-full bg-gray-100 mr-2 flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-2.5 w-2.5 text-gray-500" fill="currentColor" viewBox="0 0 20 20">
                              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-8.293l-3-3a1 1 0 00-1.414 1.414L10.586 9.5H7a1 1 0 100 2h3.586l-1.293 1.293a1 1 0 101.414 1.414l3-3a1 1 0 000-1.414z" clipRule="evenodd" />
                            </svg>
                          </span> */}
          ...
          {courseData.episodes.map((episode, index) => (
            ...
            >
              <div className="mb-4 lg:mb-6 flex justify-between items-start">
                <div>
                  <h2 className="text-xl lg:text-2xl font-bold text-gray-800 mb-2">Episode {index + 1}: {episode}</h2>
              ...
              <div className="aspect-video mb-4 lg:mb-6 rounded-lg overflow-hidden shadow-md">
                <iframe 
                  ...
                  src={courseData.videoLinks[index]} 
                  title={`Episode ${index + 1}: ${episode}`} 
                  frameBorder="0" 
                  allowFullScreen>
                </iframe>
              </div>
              
              {courseData.slideLinks[index] && (
                <div className="mb-4">
                  <a 
                    href={courseData.slideLinks[index]} 
                    target="_blank" 
                    rel="noreferrer"
              ...
              {courseData.resourceLinks[index] && courseData.resourceLinks[index].length > 0 && (
                <div className="mt-4 lg:mt-6 pt-4 lg:pt-6 border-t border-gray-200">
                  ...
                  <ul className="grid grid-cols-1 md:grid-cols-2 gap-2 lg:gap-3">
                    {courseData.resourceLinks[index].map((resource, i) => (
                      <li key={`resource-${index + 1}-${i}`} className="flex">
                        <a 
                ...
                {courseData.assignments.map((assignment, index) => (
                  <a 
                    ...
                    className={`group block p-6 rounded-lg border border-gray-200 bg-white/70 hover:bg-white hover:shadow-md transition-all duration-200 hover:border-blue-200 ${
                      index === courseData.assignments.length - 1 && courseData.assignments.length % 3 === 1 ? 'md:col-span-2 lg:col-span-1' : ''
                    }`}
                  >
